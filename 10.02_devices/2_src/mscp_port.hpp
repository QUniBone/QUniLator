/*
    mscp_port.hpp: MSCP/TMSCP controller port (SA/IP registers, init handshake,
    DMA, and command/response ring protocol).
...
// Buffers returned by mscp_port_c::DMARead() are allocated with new uint16_t[], so
...
/*
  This implements the Transport layer for a Qbus/Unibus MSCP/TMSCP controller.

  Logic for initialization, reset, and communication with the MSCP server
  is implemented here.  A concrete controller (disk or tape) supplies the
  controller-identity values, the server, and the drive objects.
*/
class mscp_port_c : public storagecontroller_c
{
public:
    mscp_port_c();
    virtual ~mscp_port_c();
...
public:
    ...
    //
    // Controller-identity seams.  Defaults are the disk (UDA50) values; a
    // concrete controller overrides them as appropriate.
    //
    // UniqueDeviceNumber / ClassModel returned by SET CONTROLLER CHARACTERISTICS.
    virtual uint32_t controller_identifier(void);
...
protected:
    //
    // Factories for the medium-specific server and drive objects.
    //
    virtual mscp_server_base *make_server(void) = 0;
    virtual storagedrive_c *make_drive(unsigned unit) = 0;
